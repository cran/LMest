                      ...
                      fort = TRUE, seed = NULL)
{
  ...
  for(kv in 1:nkv){
    out[[kv]] <- switch(model, "LMbasiccont" = (lmbasic.cont(Y = Y,k = k[kv],start = start,
                                                             modBasic = modBasic,tol = tol,
                                                             maxit = maxit,out_se = out_se,
                                                             piv = parInit$piv,Pi = parInit$Pi,
                                                             Mu = parInit$Mu,Si = parInit$Si,
                                                             miss = miss, fort = fort)),
                        "LMlatentcont" = (lmcovlatent.cont(Y = Y,X1 = Xinitial,X2 = Xtrans,
                                                           yv = freq,k = k[kv],start = start,
                                                           tol = tol,maxit = maxit,
                                                           paramLatent = paramLatent,
                                                           Mu = parInit$Mu, Si = parInit$Si,
                                                           Be = parInit$Be,Ga = parInit$Ga,
                                                           output = output, out_se = out_se,
                                                           miss = miss)))
    aicv[kv] = out[[kv]]$aic
    bicv[kv] = out[[kv]]$bic
    lkv[kv] = out[[kv]]$lk
  ...
  #   Aic = aicv
  # }
  out <- do.call(c, list(best, list(Lk = lkv, Bic = Bic, Aic = Aic, call = match.call(),data = data)))

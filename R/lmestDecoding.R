lmestDecoding <- function(est,
                         sequence = NULL, indices = NULL, fort=TRUE){
  ...
  if(class(est) == "LMbasiccont" | class(est) == "LMlatentcont")
  {
    stop("Function provide local decoding on the basis of the output of basic LM model, LM model with covariates in the latent model,
         LM model with covariates in the measurement model and mixed LM model")
  }
  ...
  data.new <- data[,-c(tv.which,id.which)]
  ...
  if(class(est)=="LMlatent"){
    ...
    if(dim(est$Psi)[3]==1){
      if(is.vector(Y)){
        Y = t(Y)
        if(miss) R = t(R)
        if(is.vector(Xinitial)) Xinitial = t(Xinitial)
        if(is.matrix(Xtrans)) Xtrans = array(Xtrans,c(1,dim(Xtrans)))
        if(is.vector(Xtrans)) Xtrans = array(Xtrans,c(1,length(Xtrans),1))
      }
      n = nrow(Y); TT = ncol(Y)
    }else{
      if(is.matrix(Y)){
    ...
    k = ncol(est$Be)+1
    Psi = est$Psi
    if(is.vector(Xinitial)) Xinitial = matrix(Xinitial,n,1)
    nc1 = dim(Xinitial)[2] # number of covariates on the initial probabilities
    Xlab = 1:n
    if(k == 2){
      GBe = as.matrix(c(0,1))
    }else{
    ...
    Piv = out$P
    if(is.matrix(Xtrans)) Xtrans = array(Xtrans,c(n,TT-1,1))
    nc2 = dim(Xtrans)[3] # number of covariates on the transition probabilities
    Z = NULL
    for(t in 1:(TT-1)) Z = rbind(Z,Xtrans[,t,])
    if(nc2==1) Z = as.matrix(Xtrans)
  ...
  if(class(est)=="LMmanifest"){
    ...
    Xmanifest = matrix(X,ne*nc,n*TT)
    out1 = t(unique(t(Xmanifest)))
    nd = ncol(out1)
    indn = rep(0,n*TT)
    INDN = vector("list",nd)
    tmp = ne*nc
    for(jd in 1:nd){
      ind = which(colSums(Xmanifest == out1[,jd])==tmp)
      indn[ind] = jd
      INDN[[jd]]$ind = ind
    }
